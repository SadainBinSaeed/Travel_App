...
const SignupPage = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen">
      {/* Left: Full image (only visible on md+) */}
      <div className="hidden md:block md:w-1/2">
        <img
          src={boyImg}
          alt="Travel"
      ...
      {/* Right: Form Section */}
      <div className="w-full md:w-1/2 relative flex items-center justify-center bg-gradient-to-br from-[#0e0f12] via-[#121418] to-[#0a0c0f] px-4 sm:px-6 lg:px-8">
        {/* Faint top text */}
        <span className="pointer-events-none select-none absolute top-2 left-1/2 -translate-x-1/2 text-white/10 font-extrabold leading-none tracking-tight text-[14vw] sm:text-[10vw] md:text-[7vw]">
          Travel
        </span>

        {/* Card */}
        <div className="w-full max-w-sm sm:max-w-md bg-transparent">
          <h2 className="text-2xl sm:text-3xl font-bold text-white text-center">
            Create an account
          </h2>
          <p className="text-xs sm:text-sm text-gray-400 text-center mt-2 mb-6">
            Enter your account details below or{" "}
          ...
          <form className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <FaUser className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm sm:text-base" />
                <input
                  type="text"
                  placeholder="Zachry"
                  className="w-full pl-10 pr-3 py-2 sm:py-3 rounded-md bg-[#f2f3f5] text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <FaEnvelope className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm sm:text-base" />
                <input
                  type="text"
                  placeholder="hello@zachry.com"
                  className="w-full pl-10 pr-3 py-2 sm:py-3 rounded-md bg-[#f2f3f5] text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
            ...
            <div>
              ...
              <div className="relative">
                <FaLock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm sm:text-base" />
                <input
                  type={showPass1 ? "text" : "password"}
                  placeholder="••••••••"
                  className="w-full pl-10 pr-20 py-2 sm:py-3 rounded-md bg-[#f2f3f5] text-black placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                />
                <button
                  type="button"
                  onClick={() => setShowPass1(!showPass1)}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-xs sm:text-sm px-2 sm:px-3 py-1 rounded-md bg-[#2b2f35] text-gray-200 hover:bg-[#3a3f46]"
                >
                  {showPass1 ? "Hide" : "View"}
                </button>
              </div>
            ...
            <button
              type="submit"
              className="w-full py-2 sm:py-3 rounded-md font-semibold bg-[#1e90ff] hover:bg-[#1676d6] text-white transition cursor-pointer text-sm sm:text-base"
            >
              <Link to="/dashboard">Signup</Link>
            </button>
          </form>
        ...
        {/* Faint bottom text */}
        <span className="pointer-events-none select-none absolute bottom-2 left-1/2 -translate-x-1/2 text-white/10 font-extrabold leading-none tracking-tight text-[12vw] sm:text-[8vw] md:text-[6vw]">
          Landseairge
        </span>
      </div>
    </div>
