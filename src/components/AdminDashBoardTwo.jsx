...
const ActionBtn = ({ kind = "approve", onClick }) => {
  const icons = {
    ...
    deny: <X size={14} />,
    preview: <Search size={14} />,
    flag: <X size={14} />, // 🚩 flag ke liye alag icon bhi use kar sakte ho (Flag import kar lo)
  };
  ...
  const styles = {
    approve:
      "bg-emerald-600 hover:bg-emerald-500 text-white border-emerald-500",
    deny: "bg-sky-700 hover:bg-sky-600 text-white border-sky-600",
    preview:
      "bg-indigo-600 hover:bg-indigo-500 text-white border-indigo-500",
    flag: "bg-rose-600 hover:bg-rose-500 text-white border-rose-500",
  };

  return (
    <button
      onClick={onClick}
      className={`h-9 px-3 rounded-md text-xs font-medium tracking-wide border transition-colors shadow-sm ${styles[kind]}`}
    >
      <span className="inline-flex items-center gap-1">
        {icons[kind]}
        {labels[kind]}
